...
function site_title() {
	global $mysqli;
	$site_title = ' BERITA TELUTCI';
	$url = $_SERVER['REQUEST_URI'];
	$explode_url = explode("/", $url);
	if (!isset($_GET['id'])) {
		switch ($explode_url[2]) {
			case '':
				$site_subtitle = 'Sumber berita terpercaya hanya disini hanya disini hanya disini';
				$title = $site_title." | ".$site_subtitle;
				break;

			case 'index.php':
				$site_subtitle = 'Sumber berita terpercaya hanya disini hanya disini';
				$title = $site_title." | ".$site_subtitle;
				break;
	...
	if (isset($_GET['p'])) {
		...
		switch ($explode_again[0]) {
			case 'index.php':
				$site_subtitle = 'Sumber berita terpercaya hanya disini';
				$title = $site_title." | ".$site_subtitle;
				break;
			case 'kategori.php':
				$site_subtitle = $data['kategori'];
